...
int diameter_of_tree(node *root)
{
   ...
   int op1 = diameter_of_tree(root->left);
   int op2 = diameter_of_tree(root->right);
   int op3 =  height_of_tree(root->right) + height_of_tree(root->left) + 1;
...
int main()
{
    ...
    cout<<"Diameter of the tree : "<<diameter_of_tree(root);
    return 0;
}